      ...
      <div className="flex-1 px-6 py-8 max-w-sm mx-auto w-full">
        <div className={`rounded-2xl p-6 text-center mb-6 ${verified ? 'bg-green-50 border border-green-200' : 'bg-red-50 border border-red-200'}`}>
          ...
          <p className={`text-sm ${verified ? 'text-green-600' : 'text-red-600'}`}>
            {verified ? 'This drug is registered with NAFDAC' : 'This drug could not be verified. Do not consume.'}
          </p>
        ...
        {!verified && (
          <div className="bg-red-50 border border-red-200 rounded-2xl p-5 mb-6">
            <h4 className="text-red-700 font-bold text-sm mb-2">What to do</h4>
        ...
        <button onClick={onBack} className="w-full bg-purple-800 hover:bg-purple-700 text-white font-bold py-3 rounded-xl text-sm mb-3 transition-all">
          Verify Another Drug
        </button>
        <button onClick={onReport} className="w-full bg-white hover:bg-gray-50 text-red-500 font-bold py-3 rounded-xl text-sm border border-red-200 transition-all">
